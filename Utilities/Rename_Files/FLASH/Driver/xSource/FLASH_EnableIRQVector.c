...
void FLASH__vEnableIRQVector(FLASH_nVECTOR_IRQ enIrqVectorArg)
{
    ...
    if((uint16_t) FLASH_enVECTOR_IRQ_RESET != (uint16_t) enIrqVectorArg)
    {
        if((uint16_t) FLASH_enVECTOR_IRQ_USER12 < (uint16_t) enIrqVectorArg)
        {
            ...
            u16IrqVectorReg <<= 1U;
            u16IrqVectorReg += FLASH_GROUP1_OFFSET + FLASH_IER_OFFSET;

            stRegister.u16Shift = u16IrqBitReg;
            stRegister.u16Mask = FLASH_GROUP_IER_IE1_MASK;
            stRegister.uptrAddress = u16IrqVectorReg;
            stRegister.u16Value = FLASH_GROUP_IER_IE1_ENA;
...
void FLASH__vDisableIRQVector(FLASH_nVECTOR_IRQ enIrqVectorArg)
{
    ...
    if((uint16_t) FLASH_enVECTOR_IRQ_RESET != (uint16_t) enIrqVectorArg)
    {
        if((uint16_t) FLASH_enVECTOR_IRQ_USER12 < (uint16_t) enIrqVectorArg)
        {
            ...
            stRegister.u16Mask = FLASH_GROUP_IER_IE1_MASK;
            stRegister.uptrAddress = u16IrqVectorReg;
            stRegister.u16Value = FLASH_GROUP_IER_IE1_DIS;

            u16InterruptStatus = MCU__u16DisGlobalInterrupt_Debug();
            FLASH__vWriteRegister(&stRegister);
            MCU__vRepeatNoOperation(#5);
            MCU__vClearInterruptStatus(u16IrqGroupReg);
            FLASH__vClearAcknowledgeIRQVector(enIrqVectorArg);

            stRegister.u16Shift = FLASH_GROUP_IER_R_IE1_BIT;
            stRegister.u16Mask = 0xFFFFU;
            stRegister.uptrAddress = u16IrqVectorReg;
            u16GroupStatus = FLASH__u16ReadRegister(&stRegister);
...
void FLASH__vDisableDebugIRQVector(FLASH_nVECTOR_IRQ enIrqVectorArg)
{
    FLASH_Register_t stRegister;
    uint16_t u16IrqVectorReg;
    uint16_t u16IrqBitReg;
...
FLASH_nENABLE FLASH__enGetEnableIRQVector(FLASH_nVECTOR_IRQ enIrqVectorArg)
{
    ...
    uint16_t u16IrqBitReg;
    uint16_t u16IrqGroupReg;
    FLASH_nENABLE enEnableReg = FLASH_enENABLE_ENA;
    ...
    if((uint16_t) FLASH_enVECTOR_IRQ_RESET != (uint16_t) enIrqVectorArg)
    {
        if((uint16_t) FLASH_enVECTOR_IRQ_USER12 < (uint16_t) enIrqVectorArg)
        {
            ...
            stRegister.u16Mask = FLASH_GROUP_IER_IE1_MASK;
            stRegister.uptrAddress = u16IrqVectorReg;
            enEnableReg = (FLASH_nENABLE) FLASH__u16ReadRegister(&stRegister);
        }
        else if((uint16_t) FLASH_enVECTOR_IRQ_RTOS >= (uint16_t) enIrqVectorArg)
        {
            u16IrqBitReg = (uint16_t) enIrqVectorArg;
            ...
            u16IrqGroupReg = 1U;
            u16IrqGroupReg <<= u16IrqBitReg;
            enEnableReg = (FLASH_nENABLE) MCU__u16GetEnaInterrupt(u16IrqGroupReg);
        }
        else
        {
